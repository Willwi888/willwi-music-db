...
Label | Willwi Music`,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const newSong: Song = {
      ...
      audioUrl: cleanAudioUrl(formData.audioUrl),
    ...
    <div className="max-w-4xl mx-auto pb-12">
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
          ...
          <div className="bg-slate-800 p-1 rounded-lg flex border border-slate-700">
              <button 
                onClick={() => { setImportMode('single'); setAlbumResults([]); setMbResults([]); setSelectedAlbum(null); setFormData(DEFAULT_FORM_DATA); }}
              ...
              <button 
                onClick={() => { setImportMode('album'); setTrackResults([]); setMbResults([]); setFormData(DEFAULT_FORM_DATA); }}
              ...
              >
               ...
               <button 
                onClick={() => { setImportMode('mb'); setTrackResults([]); setAlbumResults([]); setFormData(DEFAULT_FORM_DATA); }}
      ...
      {(importMode === 'single' || importMode === 'mb') && (
        <form onSubmit={handleSubmit} className="space-y-8 bg-slate-800 p-8 rounded-xl border border-slate-700 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <label className="block text-sm font-medium text-slate-300 mb-1">外連音源 (External Audio URL)</label>
                <input name="audioUrl" value={formData.audioUrl || ''} onChange={handleChange} className="w-full bg-slate-900 border border-slate-600 rounded p-2 text-white" placeholder="https://..." />
                <p className="text-xs text-slate-500 mt-1">支援 Dropbox 或 Google Drive 直連網址</p>
                </div>
                <div>
