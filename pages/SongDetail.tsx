...
const SongDetail: React.FC = () => {
  ...
  const [storyMode, setStoryMode] = useState<'desc' | 'story' | 'log' | 'maker'>('desc');
  ...
  const handleSave = async () => {
    if (song && id) {
      ...
      if (dataToSave.audioUrl) {
          dataToSave.audioUrl = cleanAudioUrl(dataToSave.audioUrl);
      }
      const success = await updateSong(id, dataToSave);
      if (success) {
        setSong({ ...song, ...dataToSave } as Song);
        setIsEditing(false);
        setSyncMessage('');
      } else {
      ...
      <>
        <div className="bg-slate-800 rounded-3xl overflow-hidden shadow-2xl border border-slate-700 relative">
            ...
            <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
                <div 
                    className="absolute -inset-[100%] opacity-30 mix-blend-screen animate-[spin_20s_linear_infinite]"
                    style={{
                        background: `conic-gradient(from 0deg, transparent, ${displayData.coverUrl ? 'rgba(251, 191, 36, 0.2)' : 'rgba(255,255,255,0.1)'}, transparent)`
                    }}
                />
                <div className="absolute inset-0 bg-cover bg-center opacity-40 blur-3xl scale-110 animate-pulse" style={{ backgroundImage: `url(${displayData.coverUrl})`, animationDuration: '4s' }}></div>
                <div className="absolute inset-0 bg-gradient-to-b from-brand-darker/40 via-brand-darker/80 to-brand-darker z-0"></div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
            ...
            <div className="lg:col-span-2 space-y-8">
                ...
                <div className="bg-slate-800 rounded-3xl p-8 border border-slate-700 shadow-xl">
                    <div className="flex gap-8 mb-8 border-b border-slate-700 overflow-x-auto custom-scrollbar">
                        <button 
                            onClick={() => setStoryMode('desc')}
                            className={`pb-4 text-sm font-black uppercase tracking-widest transition-all border-b-4 whitespace-nowrap ${storyMode === 'desc' ? 'border-brand-accent text-brand-accent' : 'border-transparent text-slate-500 hover:text-white'}`}
                        >
                            {t('detail_tab_story')}
                        </button>
                        <button 
                            onClick={() => setStoryMode('story')}
                            className={`pb-4 text-sm font-black uppercase tracking-widest transition-all border-b-4 whitespace-nowrap ${storyMode === 'story' ? 'border-brand-accent text-brand-accent' : 'border-transparent text-slate-500 hover:text-white'}`}
                        >
                            Creative Notes
                        </button>
                        <button 
                            onClick={() => setStoryMode('log')}
                            className={`pb-4 text-sm font-black uppercase tracking-widest transition-all border-b-4 whitespace-nowrap ${storyMode === 'log' ? 'border-brand-accent text-brand-accent' : 'border-transparent text-slate-500 hover:text-white'}`}
                        >
                            Lab Logs
                        </button>
                         <button 
                            onClick={() => setStoryMode('maker')}
                            className={`pb-4 text-sm font-black uppercase tracking-widest transition-all border-b-4 whitespace-nowrap ${storyMode === 'maker' ? 'border-brand-accent text-brand-accent' : 'border-transparent text-slate-500 hover:text-white'}`}
                        >
                            <svg className="w-4 h-4 inline-block mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
                            {t('detail_tab_maker')}
                        </button>
                    </div>
                    ...
                    {storyMode === 'story' && (
                         isEditing ? (
                            <textarea 
                                 ...
                                 placeholder="Creative notes, inspirations, behind the scenes..."
                            />
                        ) : (
                            <p className="text-slate-300 leading-relaxed whitespace-pre-line text-lg font-light">
                                {displayData.story || "No creative notes provided."}
                            </p>
                        )
                    )}
                    ...
                    {storyMode === 'log' && (
                         isEditing ? (
                            <textarea 
                                 className="w-full bg-slate-900 border border-slate-700 rounded-xl p-4 text-base text-white h-48 focus:border-brand-accent outline-none shadow-inner font-mono text-sm"
                                 value={editForm.log || ''}
                                 onChange={(e) => setEditForm({...editForm, log: e.target.value})}
                                 placeholder="Lab logs, technical details, mixing notes..."
                            />
                        ) : (
                            <p className="text-slate-400 leading-relaxed whitespace-pre-line text-sm font-mono bg-slate-900 p-6 rounded-xl border border-slate-700">
                                {displayData.log || "No lab logs provided."}
                            </p>
                        )
                    )}
                ...
                <div className="bg-slate-800/80 backdrop-blur-xl rounded-3xl p-8 border border-slate-700 shadow-xl relative overflow-hidden group">
                    <div className="absolute inset-0 bg-brand-gold/5 opacity-0 group-hover:opacity-100 transition-opacity duration-1000 pointer-events-none"></div>
                    <h3 className="text-lg font-black text-white mb-6 uppercase tracking-widest border-l-4 border-brand-accent pl-4 relative z-10">{t('detail_lyrics_header')}</h3>
                     {isEditing ? (
                        <textarea 
                             className="w-full bg-slate-900 border border-slate-700 rounded-xl p-4 text-base text-white h-[600px] font-mono leading-loose focus:border-brand-accent outline-none shadow-inner relative z-10"
                             value={editForm.lyrics || ''}
                             onChange={(e) => setEditForm({...editForm, lyrics: e.target.value})}
                        />
                    ) : (
                        <div className="text-slate-300 leading-[2.5rem] whitespace-pre-wrap font-sans text-xl font-light text-center relative z-10 transition-all duration-[2000ms] group-hover:blur-[1px] group-hover:scale-[1.01]">
...
const LyricVideoMaker: React.FC<{ song: Song }> = ({ song }) => {
    ...
    const handleSyncLine = useCallback(() => {
        ...
        setSyncData(prev => [...prev, { time: elapsedTimeRef.current, text: currentLine }]);
                    ...
                    <div className="space-y-6">
                        <div className="aspect-video bg-black rounded-2xl flex items-center justify-center relative overflow-hidden border border-white/5 shadow-2xl">
                             ...
                             <div className="relative z-10 text-center">
                                 <svg className="w-16 h-16 mb-4 text-slate-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
    ...
    return (
        <div className="relative w-full h-[700px] bg-[#0a0a0a] rounded-[2rem] overflow-hidden flex flex-col font-sans border border-white/10 shadow-2xl">
            {/* Top Bar */}
            <div className="relative z-20 flex justify-between items-center p-6 bg-gradient-to-b from-black/80 to-transparent">
                <div className="flex gap-6 text-sm">
                    ...
                    >
                        <option value="font-mono">Monospace (等寬)</option>
                        <option value="font-sans">Sans-serif (無襯線)</option>
                        <option value="font-serif">Serif (襯線)</option>
                    </select>
                    <select 
                        value={coverPosition}
                    ...
                    <select 
                        ...
                        className="bg-transparent border-none outline-none text-slate-300 cursor-pointer"
                    >
                        <option value="fade-up">淡入淡出</option>
                        <option value="none">無動態</option>
                    </select>
                </div>
                <button onClick={() => setGameState('finished')} className="text-white font-bold text-lg tracking-widest hover:text-brand-accent transition-colors">
                    匯出影片
                </button>
            </div>
            ...
            <div className="relative z-10 flex-grow flex items-center justify-center p-10">
                ...
                {/* Album Cover (Background or Side) */}
                {coverPosition === 'center' && (
                    <div className="absolute inset-0 bg-cover bg-center opacity-20 blur-xl scale-110" style={{ backgroundImage: `url(${song.coverUrl})` }}></div>
                )}
                {coverPosition === 'left' && (
                    <div className="absolute left-16 top-[40%] -translate-y-1/2 w-72 h-72 rounded-2xl overflow-hidden shadow-2xl">
                        <img src={song.coverUrl} className="w-full h-full object-cover" />
                    </div>
                )}
                {coverPosition === 'right' && (
                    <div className="absolute right-16 top-[40%] -translate-y-1/2 w-72 h-72 rounded-2xl overflow-hidden shadow-2xl">
                        <img src={song.coverUrl} className="w-full h-full object-cover" />
                    </div>
                )}

                {/* Lyrics */}
                <div className={`text-center w-full max-w-3xl z-20 ${fontFamily} flex flex-col items-center justify-center h-64 relative`}>
                    <div className={`absolute top-0 w-full text-2xl md:text-3xl text-slate-400 opacity-50 transition-all duration-500 ${animation === 'fade-up' ? 'translate-y-0' : ''}`}>
                        {lineIndex > 0 ? lyricsLines[lineIndex - 1] : ''}
                    </div>
                    <div className={`absolute top-1/2 -translate-y-1/2 w-full text-4xl md:text-5xl font-bold text-white transition-all duration-500 ${animation === 'fade-up' ? 'translate-y-0 scale-105' : ''}`}>
                        {lyricsLines[lineIndex]}
                    </div>
                    <div className={`absolute bottom-0 w-full text-2xl md:text-3xl text-slate-400 opacity-50 transition-all duration-500 ${animation === 'fade-up' ? 'translate-y-0' : ''}`}>
                        {lineIndex < lyricsLines.length - 1 ? lyricsLines[lineIndex + 1] : ''}
                    </div>
                </div>
            </div>
            ...
            <div className="absolute bottom-10 left-10 z-20">
                <button className="w-16 h-16 rounded-full border-2 border-red-500/50 flex items-center justify-center text-yellow-500 hover:bg-red-500/20 transition-colors shadow-[0_0_15px_rgba(239,68,68,0.3)]">
                    <svg className="w-8 h-8" fill="currentColor" viewBox="0 0 24 24"><path d="M9 21c0 .55.45 1 1 1h4c.55 0 1-.45 1-1v-1H9v1zm3-19C8.14 2 5 5.14 5 9c0 2.38 1.19 4.47 3 5.74V17c0 .55.45 1 1 1h6c.55 0 1-.45 1-1v-2.26c1.81-1.27 3-3.36 3-5.74 0-3.86-3.14-7-7-7zm2.85 11.1l-.85.6V16h-4v-2.3l-.85-.6A4.997 4.997 0 017 9c0-2.76 2.24-5 5-5s5 2.24 5 5c0 1.63-.8 3.16-2.15 4.1z"/></svg>
                </button>
            </div>

            {/* Bottom Right: Player Widget */}
            <div className="absolute bottom-10 right-10 w-[340px] z-20">
                <div className="bg-[#181818] rounded-2xl p-4 border border-white/10 shadow-2xl">
                    <div className="flex gap-3 items-center mb-4">
                        <img src={song.coverUrl} className="w-12 h-12 rounded object-cover shadow-md" />
                        <div className="flex-grow min-w-0">
                            <div className="text-white font-bold text-sm truncate">{song.title} {song.versionLabel && `- ${song.versionLabel}`}</div>
                            <div className="text-slate-400 text-xs truncate">Willwi</div>
                        </div>
                    ...
                    <div className="flex items-center gap-3 mb-4">
                        <div className="flex-grow h-1 bg-slate-600 rounded-full overflow-hidden cursor-pointer">
                            <div className="h-full bg-white rounded-full" style={{ width: `${(elapsedTime / (audioRef.current?.duration || 100)) * 100}%` }}></div>
                        </div>
                        <span className="text-xs text-slate-400 font-mono w-12 text-right">
                            -{audioRef.current?.duration ? formatTime(audioRef.current.duration - elapsedTime) : '0:00'}
                        </span>
                        <button className="text-white hover:text-brand-accent">
                        ...
                        <button onClick={togglePlay} className="text-white hover:text-brand-accent bg-white/10 rounded-full p-1">
                            {isPlaying ? (
                    ...
                    <button 
                        ...
                        className="w-full py-2.5 rounded-full border border-slate-500 text-white font-bold text-sm hover:bg-white hover:text-black transition-all active:scale-95"
                    >
                        同步 (SPACE)
                    </button>
                </div>
            ...
            {song.audioUrl && (
                <audio 
                    ...
                    onTimeUpdate={() => {
                        if (audioRef.current) setElapsedTime(audioRef.current.currentTime);
                    }}
                />
            )}
        </div>
